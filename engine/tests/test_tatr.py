...
def main():
  image_path = (
      r"C:\Users\chan0\Personal Project\TrustLens_v2\engine\test_doc\bank_statement_image_test_4.jpg"
  )
  ...
  print("2. 加载 Table Detection 模型并在拉正图上定位主表...")
  ...
  for score, label, box in zip(
    ...
    if det_model.config.id2label[label.item()] == "table":
      table_boxes.append([round(i, 2) for i in box.tolist()])
  ...
  print(f"   => 命中主表格物理坐标: [{tx1}, {ty1}, {tx2}, {ty2}]")
  ...
  print("3. 加载 Structure Recognition 模型解析行列网格...")
  ...
  with open("table_output_cropped_2.json", "w", encoding="utf-8") as f:
  ...
  image.save("table_grid_cropped_visualized_2.jpg")
  print(
      "✅ 运行完成！拉正后的结果已保存至 table_grid_cropped_visualized.jpg 和"
      " table_output_cropped.json"
  )
